...
UNINSTALL_SELECTION=$(yad --list --checklist --title="Nix Manager - Uninstall" --text="Select applications to uninstall:" \
...
INSTALL_SELECTION=$(yad --list --checklist --title="Nix Manager - Install" --text="Select applications to install:" \
...
for APP in "${UNINSTALLING[@]}"; do
  ...
  yad --info --text="Uninstalling $APP" --width=500 --height=200
  (nix profile remove $APP | tee >(yad --progress --title="Uninstalling $APP" --text="Uninstalling..." --pulsate --auto-close --width=500 --height=200)) || show_error "Error uninstalling $APP"
done

# Process installation of selected apps
for APP in "${INSTALLING[@]}"; do
  echo "Installing $APP"
  yad --info --text="Installing $APP" --width=500 --height=200
  (nix profile install "nixpkgs#$APP" | tee >(yad --progress --title="Installing $APP" --text="Installing..." --pulsate --auto-close --width=500 --height=200)) || show_error "Error installing $APP"
done
