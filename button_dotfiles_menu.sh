...
yad --title="" \
  ...
  --field="<span foreground='#00FF00'>$(printf '\uF0C5')</span>     Source pwsh profile.ps1":FBTN "pwsh -c \"if (-not (Get-Content $PROFILE.CurrentUserAllHosts | Select-String -Pattern '\\. \\\"$env:HOME/Github/dotfiles/pwsh/profile.ps1\\\"')) { Add-Content -Path $PROFILE.CurrentUserAllHosts -Value '. \\\"$env:HOME/Github/dotfiles/pwsh/profile.ps1\\\"'; Write-Host -ForegroundColor Green 'pwsh profile has been copied!' } else { Write-Host -ForegroundColor Yellow 'pwsh profile already exists!' }\"" \
