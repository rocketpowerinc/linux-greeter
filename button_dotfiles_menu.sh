...
yad --title="" \
  ...
  --field="<span foreground='#00FF00'>$(printf "\uF0C5")</span>     Source pwsh profile.ps1":FBTN "bash -c 'grep -qxF \"source \\\"$DOTFILES/pwsh/profile.ps1\\\"\" $PROFILE.CurrentUserAllHosts || echo \"source \\\"$DOTFILES/pwsh/profile.ps1\\\"\" >> ~/.bashrc; echo -e \"\e[32mpwsh profile has been copied!\e[0m\"'" \
