...
yad --title="" \
  ...
  --field="<span foreground='#00FF00'>$(printf "\uF0C5")</span>     Source zshrc":FBTN "bash -c 'grep -qxF \"source \\\"$DOTFILES/zsh/zshrc\\\"\" ~/.zshrc || echo \"source \\\"$DOTFILES/zsh/zshrc\\\"\" >> ~/.zshrc; echo -e \"\e[32m.zshrc has been copied!\e[0m\"'" \
  --field="<span foreground='#00FF00'>$(printf '\uF0C5')</span>     Source pwsh profile.ps1":FBTN "pwsh -c \"if (-not (Get-Content $HOME/.config/powershell/profile.ps1 | Select-String -Pattern '\\. \\\"$env:HOME/Github/dotfiles/pwsh/profile.ps1\\\"')) { Add-Content -Path $HOME/.config/powershell/profile.ps1 -Value '. \\\"$env:HOME/Github/dotfiles/pwsh/profile.ps1\\\"'; Write-Host -ForegroundColor Green 'pwsh profile has been copied!' } else { Write-Host -ForegroundColor Yellow 'pwsh profile already exists!' }\"" \
